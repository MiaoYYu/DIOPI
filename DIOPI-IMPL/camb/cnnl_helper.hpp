...
#define DIOPI_CALLCNNL(Expr)                                                                                                                                  \
    do {                                                                                                                                                      \
        ...
        if (ret != ::CNNL_STATUS_SUCCESS) {                                                                                                                   \
            setLastErrorString("cnnl error %d : %s at %s:%d, cnrt error msg: %s.", ret, ::cnnlGetErrorString(ret), __FILE__, __LINE__, ::cnrtGetLastError()); \
...
#define DIOPI_CHECKCNNL(Expr)                                                                                                                     \
    do {                                                                                                                                          \
        ::cnnlStatus_t ret = Expr;                                                                                                                \
        if (ret != ::CNNL_STATUS_SUCCESS) {                                                                                                       \
            printf("cnnl error %d : %s at %s:%d, cnrt error msg: %s.", ret, ::cnnlGetErrorString(ret), __FILE__, __LINE__, ::cnrtGetLastError()); \
            std::abort();                                                                                                                         \
        }                                                                                                                                         \
    } while (false);
